...
class Tools
{
    ...
    public static function doRequest($method, $url, $options = [])
    {
        ...
        } else throw new InvalidArgumentException("Certificate file that do not exist. --- [ssl_cer]");
        // 证书文件设置
        if (!empty($options['ssl_key'])) if (file_exists($options['ssl_key'])) {
            curl_setopt($curl, CURLOPT_SSLKEYTYPE, 'PEM');
            curl_setopt($curl, CURLOPT_SSLKEY, $options['ssl_key']);
        } else throw new InvalidArgumentException("Certificate file that do not exist. --- [ssl_key]");
